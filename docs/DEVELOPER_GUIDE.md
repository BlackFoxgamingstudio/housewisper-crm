# Developer Guide: Sovereign HouseWisper CRM
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/housewisper-crm.git
cd housewisper-crm
...
```bash
export SBB_MLS_API_KEY=your_value_here
export SBB_CRM_WEBHOOK_SECRET=your_value_here
export SBB_VALUATION_MODEL=your_value_here
export SBB_CALENDAR_PROVIDER=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-housewisper-crm)
...
```bash
# Build
docker build -t sovereign-housewisper-crm:dev .

# Run with env file
docker run --env-file .env -p 8807:8807 sovereign-housewisper-crm:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-housewisper-crm
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8807`

### Triggering Directly

```bash
curl -X POST http://localhost:8807/api/v1/execute \
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-housewisper-crm",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8807` in use | `lsof -i :8807` | Kill PID or change `SERVICE_PORT` |
